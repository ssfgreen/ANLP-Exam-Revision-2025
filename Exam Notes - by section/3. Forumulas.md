...
## 5️⃣ DOT PRODUCT & COSINE SIMILARITY
...
- Invariant to **vector length** → good for embeddings where frequency affects norms.
...
- Doesn’t encode all linguistic relations (e.g. antonyms can be close).
...
**Interpretation:** They point in almost the same direction → very similar meanings.
...
True word is **“cat”**. Gold distribution is one-hot:
...
**Interpretation:** The model was somewhat surprised (0.2 prob). Lower cross-entropy → better model.

---

# 🔟–1️⃣4️⃣ EXTENDED-STYLE WORKED EXAMPLES
...
**Interpretation:** They are **colinear** in embedding space → extremely similar.
...
**Interpretation:** “goal” is an extremely strong indicator of **sports**.
...
In political text, “prime” and “minister” almost always co-occur; their probabilities are **not independent** given the politics class.
...
**Interpretation:** Probability is pushed **way down** because we’re sharing counts with 5000 vocabulary items.
...
**Kneser–Ney smoothing**: discounts counts and backs off using **continuation probabilities** (how many different contexts a word appears in), giving much more realistic behaviour.

---

### **13. Precision / Recall Trade-Off**

Threshold ↑ → stricter criteria for positive.

**(a) Why does precision ↑ and recall ↓?**

- **Higher threshold:** only very confident positives are accepted.
- Fewer false positives → **precision increases**.
- But more true positives get rejected → **recall decreases**.
...
Automatic legal/medical decisions: accusing people of hate speech or misdiagnosing disease should be rare → **false positives are very costly**.
...
- Minimising $L = -\log q(\text{gold})$
  ↔ maximising $\log q(\text{gold})$.
- Summed over dataset, this is exactly **maximum likelihood estimation**.

**(c) When is cross-entropy not a good evaluation metric?**

For **open-ended generation quality** (e.g. long-form dialogue, summarisation readability). Cross-entropy doesn’t directly capture **fluency, coherence, faithfulness**; you use BLEU/ROUGE/human judgements instead.
