...
Same blocks (self-attn + FFN), but with **causal mask** → each token can only attend to past and itself.
...
Same attention math as above, but:

In $QK^\top$, entries referring to **future positions** ($j > i$) are masked to $-\infty$ before softmax.

One LM step (conceptually):

1. Input tokens $x_{1..t}$ → embeddings + positions.
2. Run through all layers with causal attention.
3. Take final hidden state at position $t$, project via output head → logits over vocab.
4. Softmax → distribution over next token.

## **B. Parameter Counting**

Same structural components as encoder-only:

- **Embedding matrix**: $|V| \times d$.
- **Layer stack**: for each layer, attention + FFN params.
- Often **weight tying** between embeddings and output projection.

Rough scaling:

$$
\text{Total params} \approx \text{embeddings} + L \cdot (c_1 d^2 + c_2 d d_{\text{ff}})
$$

with constants from projections.

In exam: be able to explain which matrices contribute and approximate totals.

## **C. Training + Regularisation**

**Objective**:

- Next-token cross-entropy on large text corpus.

**Teacher forcing**:

- During training, always feed **true previous tokens**, not model predictions.

**Regularisation**:

- Dropout, LayerNorm.
- Sometimes weight decay.
- KV cache is an inference-time optimisation, not regularisation.

## **D. Typical Tasks**

- **Text generation / continuation**.
- All tasks framed as "predict the right text given prompt":
  - QA, translation, summarisation, classification via prompts, etc.
...
- Very rich generative behaviour, long-range context (within context window).

**Can't**:

- See future context in a single pass (causal).
- Efficiently handle extremely long sequences beyond window (though extended with tricks).

## **F. Pros / Cons**

**Pros**: universal text generator; flexible via prompting; great for generative tasks.

**Cons**: expensive to pretrain; still $O(N^2)$; lacks explicit bidirectional context in a single pass.

## 🔟 Transformer Encoder-Decoder (T5-style)

**Core idea**

- **Encoder**: like BERT encoder (self-attention).
- **Decoder**: like GPT decoder (masked self-attention) plus **cross-attention** to encoder outputs.
- Often used in text-to-text framing (T5).

## **A. Computational / Forward Step**

**Encoder**:

Input tokens → embeddings + positions → stacked self-attn+FFN layers → encoder states $H_{\text{enc}}$.

**Decoder per step**:

1. **Masked self-attention** over previous output tokens.
...
3. Attention$(Q, K, V)$ as usual.
4. FFN, residuals, LayerNorm.
5. Final decoder hidden state → unembedding → logits over vocab.

## **B. Parameter Counting**

Total params ≈ encoder stack + decoder stack + embeddings.

Roughly:

- **Encoder**: $L_{\text{enc}} \times$ (attention + FFN params).
- **Decoder**: $L_{\text{dec}} \times$ (self-attn + cross-attn + FFN params).
- **Embeddings** (and possibly shared with LM head).

You should be able to explain why encoder–decoder generally has more params than encoder-only or decoder-only with same depth.

## **C. Training + Regularisation**

**Pretraining (T5-style)**:

- Denoising / span corruption: mask spans and predict them (via special sentinel tokens).

**Fine-tuning**:

- Each task cast as text-to-text ("translate: …", "summarise: …").

**Regularisation**: dropout, LayerNorm, etc.

## **D. Typical Tasks**

- Machine translation.
- Summarisation.
- QA, text editing, other seq2seq tasks.
- In T5, basically anything text-to-text (classification → output label as text).

## **E. What It Can / Can't Capture**

**Can**:

- Very flexible mapping from input text → output text.
- Encoder can specialise in representing inputs; decoder specialises in generation.

**Can't**:

- Efficiently scale to huge seq lengths without same $O(N^2)$ issues.
- Avoid high compute; two stacks are heavier.

## **F. Pros / Cons**

**Pros**: strong for supervised seq2seq; unified text-to-text formulation (T5).

**Cons**: heavier than encoder-only for classification; more complex than pure decoder-only LM.
