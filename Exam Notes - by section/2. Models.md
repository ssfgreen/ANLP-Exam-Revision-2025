...
Each head has a dimension $d_k$ which $= \frac{h}{d}$ with $h$ heads, the concatenated output is $hd_k$ = $d$. $W_O$ mixes head outputs back to the correct shape to be able to have layerNorm + Residuals applied.
...
If this math formatting looks good to you, I’ll **next**:

- Clean up **decoder-only (GPT)** and **encoder–decoder (T5)** in the _same_ MathJax style,
- And we can then slot them directly into your master revision doc.
