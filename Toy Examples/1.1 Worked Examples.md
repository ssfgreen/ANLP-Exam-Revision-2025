...
I’ll label them **Example 1, 2, …** so you can turn them into your own Q→A cards.

---

## 1️⃣ N-GRAM LM: MLE + ADD-α + SENTENCE PROBABILITY
...
> (a) MLE, (b) add-α with $\alpha = 0.1$, assuming vocab size $|V| = 10$ tokens (incl. `<s>`, `</s>`).
...
#### (b) **Add-α smoothing (α = 0.1)**

We assume every possible continuation from `<s>` gets +α:
...
## 3️⃣ BINARY LOGISTIC REGRESSION (SENTIMENT)

### Setup

Two sentences:

1. “the dog sees the cat” → label **y=1** (positive)
2. “the cat chases a dog” → label **y=0** (negative)
...
Sentence 1 features: dog ✓, cat ✓, chases ✗, quickly ✗  
So $x = [1, 1, 0, 0]$.
...
In exam: you can stop at “>0 → P>0.5 → positive”.

---

## 4️⃣ MULTINOMIAL LOGISTIC REGRESSION (SOFTMAX)
...
Features: dog=1, cat=1, chases=1 → $x=[1,1,1]$.
...
Highest score: **class 2 (ACTION)** → most probable.

---

## 5️⃣ SKIP-GRAM WITH NEGATIVE SAMPLING

We use small 2D embeddings:

- $v(\text{dog}) = [1, 0]$ – target embedding
- $u(\text{chases}) = [0.5, 0.5]$ – context embedding
- $u(\text{cat}) = [-0.5, 0.5]$ – negative context embedding
...
You don’t need exact decimals in exam—just show the structure.

---

## 6️⃣ MLP (FEED-FORWARD) ONE STEP
...
## 7️⃣ RNN: ONE STEP OF HIDDEN STATE
...
That’s the basic “by hand” step.

---

## 8️⃣ ATTENTION: ONE QUERY, FULL WEIGHTS
...
You don’t need numeric approximations; the structure is enough.

---

## 9️⃣ TRANSFORMER ENCODER PARAM COUNT (ONE LAYER)
...
- Output projection ($W_O$): $4 \times 4 = 16$  
   → total attention weights = 48 + 16 = **64**
...
Ignoring biases & LayerNorm params (small), total ≈  
...
Show this, and you’ve demonstrated **top-k**.
...
## 1️⃣2️⃣ VECTOR SIMILARITY: DOT, EUCLIDEAN, COSINE
...
## 1️⃣3️⃣ CROSS-ENTROPY & PERPLEXITY
...
## 1️⃣4️⃣ BPE ON “the dog doghouse”
...
1. Count most frequent **adjacent pairs** across all words (e.g., `d o`, `o g`, `d o`, `o g`, `g h`, …).
2. Merge that pair into a new symbol (say “do”).
3. Repeat until desired vocab size reached.

> **Exam style:** You’d be given a tiny table of counts and asked:
>
> - which pairs are merged in which order,
> - final vocab,
> - how to tokenise “doghouse\_”.

Use our toy data to simulate a couple merges by hand exactly as in Tutorial 4 / exam practice style.

---

## 1️⃣5️⃣ PRECISION / RECALL / F1
